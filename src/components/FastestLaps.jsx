...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-f1card border border-f1border rounded p-3 shadow-xl">
      <p className="text-white font-display font-semibold text-sm">{d.fullName}</p>
      <p className="text-f1muted font-mono text-xs">{d.team}</p>
      <div className="mt-2 space-y-1">
        <div className="flex justify-between gap-4"><span className="text-f1muted font-mono text-xs">Time</span><span className="number-font text-white text-xs font-bold">{d.timeStr}</span></div>
        <div className="flex justify-between gap-4"><span className="text-f1muted font-mono text-xs">Lap</span><span className="number-font text-f1muted text-xs">{d.lap}</span></div>
        <div className="flex justify-between gap-4"><span className="text-f1muted font-mono text-xs">Speed</span><span className="number-font text-f1muted text-xs">{d.speed} km/h</span></div>
      </div>
    </div>
  );
};
...
export default function FastestLaps() {
  ...
  if (loading) return <div className="f1-card p-4"><SectionHeader title="Fastest Laps" accent /><div className="h-64 shimmer rounded mt-4"/></div>;
  ...
  return (
    <div className="space-y-4 animate-slide-up">
      <div className="f1-card p-4">
        <SectionHeader title="Fastest Laps" subtitle={data.race?.raceName ?? ''} accent/>
        <div className="flex items-center gap-3 mb-4 p-3 bg-f1red/10 border border-f1red/20 rounded">
          <span className="text-2xl">⚡</span>
          <div>
            <p className="text-white font-display font-bold text-sm">{chartData[0]?.fullName}</p>
          ...
          <div className="ml-auto text-right">
            ...
            <p className="text-f1muted font-mono text-xs">Fastest Lap · Lap {chartData[0]?.lap}</p>
          </div>
        </div>
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={chartData} layout="vertical" margin={{top:0,right:50,left:10,bottom:0}}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1E1E1E" horizontal={false}/>
            <XAxis type="number" domain={[bestTime-0.5,'auto']} tickFormatter={formatLapTime} tick={{fill:'#888',fontSize:10,fontFamily:'JetBrains Mono'}}/>
            <YAxis type="category" dataKey="driver" width={35} tick={{fill:'#ccc',fontSize:11,fontFamily:'JetBrains Mono'}}/>
            <Tooltip content={<CustomTooltip/>}/>
            <Bar dataKey="time" radius={[0,3,3,0]} barSize={12}>
            ...
            <tbody>
              {chartData.map((row,i)=>(
                <tr key={row.driver} className="border-b border-f1border/50 hover:bg-f1border/20 transition-colors">
                  <td className="px-4 py-2.5"><span className={`number-font font-bold text-sm ${i===0?'text-f1red':'text-f1muted'}`}>{i+1}</span></td>
                  ...
                  <td className="px-4 py-2.5 text-right"><span className={`number-font text-xs font-bold ${i===0?'text-f1red':'text-white'}`}>{row.timeStr}</span></td>
                  <td className="px-4 py-2.5 text-right"><span className="number-font text-f1muted text-xs">{i===0?'—':`+${row.delta.toFixed(3)}`}</span></td>
                  <td className="px-4 py-2.5 text-right hidden sm:table-cell"><span className="number-font text-f1muted text-xs">{row.lap}</span></td>
                  <td className="px-4 py-2.5 text-right hidden md:table-cell"><span className="number-font text-f1muted text-xs">{row.speed}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
